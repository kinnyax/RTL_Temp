...
module ADC_SYNC(
    ...
    input                                   sysref                                         ,
    input               [ 7:0]              link_ready_adc                                 ,
    input               [ 7:0]              fifo_empty_adc                                 ,
    input               [ 7:0]              afe_idle_adc                                   ,
    input               [ 7:0]              pll_lock_adc                                   ,
    input               [ 7:0]              rx_reset_done_adc                              ,
    input               [15:0]              lane_ready_adc                                 ,
    input               [15:0]              byte_aligned_adc                               ,
    input               [15:0]              comma_detected_adc                             ,
    input               [ 7:0]              link_error_evt_adc                             ,
    input               [ 7:0]              sysref_seen_evt_adc                            ,
    input               [15:0]              disparity_evt_adc                              ,
    input               [15:0]              notintable_evt_adc                             ,
    input               [ 7:0]              fifo_overflow_evt_afe                          ,
    input               [ 7:0]              fifo_full_afe                                  ,
    input               [ 7:0]              data_error_evt_afe                             ,
    ...
    input               [ 7:0]              tgc_cmd_evt_sys                                ,
    input               [15:0]              tgc_profile_sys                                ,
    input               [ 7:0]              tgc_up_dn_sys                                  ,
    input               [ 7:0]              tgc_done_evt_afe                               ,
...
levels_sync #(.DS(8),.RV(1'd0)) link_ready_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(link_ready_adc),.out(link_ready_sys));
levels_sync #(.DS(8),.RV(1'd1)) fifo_empty_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(fifo_empty_adc),.out(fifo_empty_sys));
levels_sync #(.DS(8),.RV(1'd0)) fifo_full_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(fifo_full_afe),.out(fifo_full_sys));
levels_sync #(.DS(8),.RV(1'd1)) afe_idle_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(afe_idle_adc),.out(afe_idle_sys));
levels_sync #(.DS(8),.RV(1'd0)) pll_lock_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(pll_lock_adc),.out(pll_lock_sys));
levels_sync #(.DS(8),.RV(1'd0)) rx_reset_done_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(rx_reset_done_adc),.out(rx_reset_done_sys));
levels_sync #(.DS(16),.RV(1'd0)) lane_ready_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(lane_ready_adc),.out(lane_ready_sys));
levels_sync #(.DS(16),.RV(1'd0)) byte_aligned_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(byte_aligned_adc),.out(byte_aligned_sys));
levels_sync #(.DS(16),.RV(1'd0)) comma_detected_cdc(.clk(sys_clk),.rst_n(sys_rst_n),.in(comma_detected_adc),.out(comma_detected_sys));

levels_sync #(.DS(16),.RV(1'd0)) tgc_profile_cdc(.clk(afe_clk),.rst_n(afe_rst_n),.in(tgc_profile_sys),.out(tgc_profile_afe));
levels_sync #(.DS(8),.RV(1'd0)) tgc_up_dn_cdc(.clk(afe_clk),.rst_n(afe_rst_n),.in(tgc_up_dn_sys),.out(tgc_up_dn_afe));
...
pulse_sync2 fifo_overflow0_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[0]),.out(fifo_overflow_evt_sys[0]));
pulse_sync2 fifo_overflow1_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[1]),.out(fifo_overflow_evt_sys[1]));
pulse_sync2 fifo_overflow2_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[2]),.out(fifo_overflow_evt_sys[2]));
pulse_sync2 fifo_overflow3_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[3]),.out(fifo_overflow_evt_sys[3]));
pulse_sync2 fifo_overflow4_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[4]),.out(fifo_overflow_evt_sys[4]));
pulse_sync2 fifo_overflow5_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[5]),.out(fifo_overflow_evt_sys[5]));
pulse_sync2 fifo_overflow6_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[6]),.out(fifo_overflow_evt_sys[6]));
pulse_sync2 fifo_overflow7_cdc(.clka(afe_clk),.clkb(sys_clk),.rst_n_a(afe_rst_n),.rst_n_b(sys_rst_n),.in(fifo_overflow_evt_afe[7]),.out(fifo_overflow_evt_sys[7]));
