...
wire                                        link_error_evt_sync                            ;


level_sync phy_rx_reset_done_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(phy_rx_reset_done),.out(phy_rx_reset_done_sync));
level_sync phy_pll_lock_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(phy_pll_lock),.out(phy_pll_lock_sync));
levels_sync #(.DS(2),.RV(1'd0)) phy_byte_aligned_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(phy_byte_aligned),.out(phy_byte_align_sync));
level_sync link_ready_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(link_ready),.out(link_ready_sync));
levels_sync #(.DS(2),.RV(1'd0)) lane_ready_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(lane_ready),.out(lane_ready_sync));
levels_sync #(.DS(2),.RV(1'd0)) cgs_ready_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(cgs_ready),.out(cgs_ready_sync));
...
pulse_sync2 link_error_cdc(.clka(jesd_clk),.clkb(adc_clk),.rst_n_a(jesd_rst_n),.rst_n_b(adc_rst_n),.in(link_error),.out(link_error_evt_sync));
...
level_sync #(.RV(1'd1)) upk_idle_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(upk_idle),.out(upk_idle_sync));
level_sync #(.RV(1'd1)) tgc_idle_cdc(.clk(adc_clk),.rst_n(adc_rst_n),.in(tgc_idle),.out(tgc_idle_sync));

assign link_error_sync = sysref_error_sync | link_error_evt_sync;
